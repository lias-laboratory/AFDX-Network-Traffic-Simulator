...
Example:
    python3 generator.py --topology tree --nb_switch 7 --nb_end_system 20 --nb_flux 15 --seed 40
...
def bag_us(debut, fin):
    """
   eturns the bags (in µs) of the form 2^k, with:
    ...
    k_min = math.ceil(math.log2(debut))
    k_max = math.floor(math.log2(fin))
...
class generator:
    ...
    Parameters:
        ...
        nb_flux            : number of flows to generate (default: 3 * nb_switch)
    ...
    def __init__(self,seed:int,nb_flux=(int,int),policy_service:str=None,
    ...
    def additional_edges(self, n: int, aretes_exist: set, k: int) -> set:
    ...
    def generate_end_systems(self,switches):
        ...
        #--------------- Tree Topology ------------------- 
            ...
            leaves = getattr(self, "_feuilles_tree", None) or [switches[-1].id]
        ...
        #--------------- lineNI/NO Topology ------------------- 
            ...
            for i in range(1, self.Nes + 1):
                ...
                if role == "source":
                    sw_id = switches_ids_src[coun_src % nb_sw_src]
                    coun_src += 1
                    es = End_System(
                        id=f"ES{i}",
                        output_port=Output_port(
                ...
                #If the role is "destination," we do this.
                    ...
                    sw_id = switches_ids_dst[coun_dst % nb_sw_dst]
                    coun_dst += 1
                    es = End_System(
                        id=f"ES{i}",
                        output_port=Output_port(
        ...
        #--------------- Random Topology -------------------
                ...
                switches_a_couvrir = switches_ids.copy()
                self.rng.shuffle(switches_a_couvrir)
        
                i = 1
                for sw_id in switches_a_couvrir:
                    if i > self.Nes:
                        break
                    role = roles[i - 1]
    ...
    def construct_pairs(self, nb_flux, paires_valides):
        """
        Constructs a list of `nb_flux` (source, destination) pairs from `paires_valides`, ensuring maximum coverage:
        ...
        while len(flux_pairs) < nb_flux and paires_valides:
            #Randomly select any valid pair
            es_src, es_dst = self.rng.choice(paires_valides)
            #Add the pair
            flux_pairs.append((es_src, es_dst))

        #Return only the first nb_flux pairs
        return flux_pairs[:nb_flux]
    ...
    def generate_flows(self, end_systems):
        """
        Generates nb_flux flows, ensuring that each end system specified by the user is used at least once as a source or destination.
        """
        ...
        if self.topology in ("single_node", "line1I/1O", "lineNI/1O", "tree"):
            ...
            for i, (es_src, es_dst) in enumerate(self.construct_pairs(nb_flux, paires_valides)):
        ...
        # ---- ring / random :  multiple sources and destinations -----------
            ...
            if self.topology == "ring":
                ...
                paires_valides = [
                    (s, d) for s in src for d in dst
                    if 1 <= distance_circulaire(s.sw_id, d.sw_id) <= dist_max
                ]
                if not paires_valides:
                    raise ValueError(
    ...
    def check_admit(self, ports_id, interference, rho_min):
        """
        ...
        for pid, list_flow_ids in interference.items():
            ...
            charge_min = N_j * rho_min
            #Check if total minimum load must not exceed port capacity
            if charge_min > Rj:
                admit = False
                print(
                    f"[ERROR] Port '{pid}' cannot be used: {N_j} flows cross it,requiring a minimum load of {charge_min:.1f} Mbps "
    ...
    def controle_admission(self, switches: List[Switch], flows: List[Flow],interference: Dict[str, List[str]]):
        ...
        One simply needs to select L within the interval [Lmin, L_max], where:
            ...
            L_max = min(Lmax, floor(capacite_disponible * bagmax / 8))
 
        and then calculate the smallest valid bag >= L*8/capacite_disponible (which will automatically be <= bagmax by design). 
        This AVOIDS any capacity-related rejections.
        """
        ports_id = {p.id: p for sw in switches for p in sw.output_port}
        flow_par_id = {f.id: f for f in flows}
        ...
        if not self.check_admit(ports_id, interference, rho_min):
            raise ValueError(
                "Admission control aborted: with the current -L/-BAG/-R parameters, "
                "at least one port cannot admit all the flows routed through it "
        ...
        def tir_L_bag(capacity_dispo: float):
            """
            Draw a valid (L_i, bag_i, rho_i) that respects capacite_dispo,
            bounding L_i in advance to guarantee that a solution exists. 
            Returns (L_i, bag_i, rho_i). Can only fail if capacite_dispo < rho_min.
            """
            R_dispo = max(capacity_dispo, rho_min)
            #Maximum frame size that can be supported with BAGmax
            L_max= min(Lmax, int((R_dispo * bagmax) / 8))
        ...
        for pid in ports_tries:
            #Retrieve the list of flows for each port.
            liste_flow_ids = interference[pid]
            #The number of flows in each port
            N_j = len(liste_flow_ids)
            ...
            #Try to admit the remaining flows traversing this port
            for flid in liste_flow_ids:
                ...
                R_now[pid] += rhoi
                flow_admis_ids.add(flid)
                k += 1
        
        #Recover the accepted and rejected Flows
        flow_admis = [flow_par_id[flid] for flid in flow_admis_ids]

        #Affects the load on all switch ports
        for pid, charge in R_now.items():
            port = ports_id[pid]
            #Load in Mbps
            port.charge_mbps = charge
            #Load as a percentage of the port capacity.
            port.charge_porcentage = (
                charge / port.bandwidth * 100.0 if port.bandwidth > 0 else 0.0
            )
        #Return the list of admitted flows
        return flow_admis
    ...
    def generer(self)->configuration:
        ...
        flux_admis= self.controle_admission(switches, flows, interference)

        #Combine accepted and rejected flows for complete record
        toutes_les_flux = flux_admis

        #Return a configuration object containing all generated elements
        return configuration(type=self.topology, switches=switches, end_systems=end_systems, Flows=toutes_les_flux)
    ...
    for sw in configuration.switches:
        ...
        for port in sw.output_port:
            ...
            port_dict = {
                ...
                "load_mbps": port.charge_mbps,
                "load_porsentage":port.charge_porcentage
            }
...
def validate_parameters(topology: str,nb_switch: int,nb_es: int,nb_flux: int,size: Optional[tuple] = None,bag: Optional[tuple] = None,bandwidth: Optional[float] = None,
    ...
    Returns: erreurs: list of error messages. topology: possibly topology. bag: possibly BAG interval
    """
    ...
    if topology == "single_node" and nb_switch >= n_min_sw:
       ...
       #At least one flow is required per source.
        if nb_flux < nb_es - 1:
            erreurs.append(
                    f"Nfl={nb_flux} insufficient for 'single_node' with Nes={nb_es}: at least {nb_es - 1} flows required (1 per source)."
            )
    ...
    if topology == "line1I/1O" and nb_switch >= n_min_sw:
        ...
        if nb_flux < nb_es - 1:
            erreurs.append(
                   f"Nfl={nb_flux} insufficient for 'line1I/1O' with Nes={nb_es}: at least {nb_es - 1} flows required (1 per source)."
            )
    ...
    if topology == "lineNI/1O" and nb_switch >= n_min_sw:
        ...
        if nb_flux < nb_es - 1:
            erreurs.append(
                    f"Nfl={nb_flux} insufficient for 'lineNI/1O' with Nes={nb_es}: at least {nb_es - 1} flows required (1 per source)."
            )
   ...
   #Check the minimum number of end systems and flows for lineNI/NO
    if topology =="lineNI/NO" and nb_switch >= n_min_sw:
        ...
        if nb_flux < (nb_es + 1) // 2:
            erreurs.append(
                    f"Nfl={nb_flux} insufficient for 'lineNI/NO' with Nes={nb_es}: at least { (nb_es + 1) // 2 } flows required (1 per source)."
            )
    ...
    if topology =="tree" and nb_switch >= n_min_sw:
        ...
        if nb_flux < nb_es - 1:
            erreurs.append(
                    f"Nfl={nb_flux} insufficient for 'tree' with Nes={nb_es}: at least {nb_es - 1} flows required (1 per source)."
            )
        elif nb_flux < nb_es_tree_min - 1:
            erreurs.append(
                    f"Nfl={nb_flux} insufficient for 'tree' with Nes={nb_es}: at least {nb_es_tree_min - 1} flows required (1 per source)."
                )
    ...
    if topology == "ring" and nb_switch >= n_min_sw:
        ...
        if nb_flux < (nb_es + 1) // 2:
            erreurs.append(
                    f"Nfl={nb_flux} insufficient for 'ring' with Nes={nb_es}: at least { (nb_es + 1) // 2 } flows required (1 per source)."
            )
    ...
    if topology == "random" and nb_switch >= n_min_sw:
        ...
        if nb_flux < (nb_es + 1) // 2:
            erreurs.append(
                   f"Nfl={nb_flux} insufficient for 'random' with Nes={nb_es}: at least { (nb_es + 1) // 2 } flows required (1 per source)."
            )
    ...
    if size is not None:
        Lmin, Lmax = size
        if Lmin <= 0 or Lmax <= 0:
            erreurs.append(f"-L={size} invalid: sizes must be > 0.")
        elif Lmin > Lmax:
            erreurs.append(f"-L={size} invalid: Lmin ({Lmin}) > Lmax ({Lmax}).")
    ...
    if bag is not None:
        ...
        if liste_bag and (liste_bag[0] != Bmin or liste_bag[-1] != Bmax):
            if liste_bag[0]>=128 and liste_bag[-1] <=16384:
                ...
                bag=[128,16384]
        if Bmin <= 0 or Bmax <= 0:
            erreurs.append(f"bag={bag} invalid: values must be > 0.")
        elif Bmin > Bmax:
            erreurs.append(f"bag={bag} invalide : Bmin ({Bmin}) > Bmax ({Bmax}).")
        else:
    ...
    if policy is not None and policy not in ("FIFO", "FP/FIFO"):
        erreurs.append(f"-Policy={policy!r} invalid: possible values 'FIFO' or 'FP/FIFO'.")
    #Validate the maximum priority.
    if priority_max is not None and priority_max < 0:
        erreurs.append(f"priority_max={priority_max} invalid: maximum priority must be >= 0.")
...
def main():      
    ...
    def parse_interval(interval_str):
        ...
        try:
         ...
         raise argparse.ArgumentTypeError(
            f"'{interval_str}' invalide : les bornes doivent être des nombres entiers."
          )
        if len(parts) == 1:
            val = int(parts[0])
            return (val, val)
    ...
    erreurs ,topology,bag= validate_parameters(
        topology=args.topology,
        nb_switch=args.nb_switch,
        nb_es=args.nb_end_system,      
        nb_flux=args.nb_flow,
        size=size,
        bag=bag_range,
